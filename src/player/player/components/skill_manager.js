export const PlayerSkillEvents = {
...
export class PlayerBuffManager {
    ...
    addBuff(typeId, params) {
        if(this.skillMap.has(typeId))return false;
        const id=this.player.events.OnBuffAddedRequest?.(typeId, params);
        if(id == null)return false;
        this.skillMap.set(typeId, id);
        return true;
    }
     ...
     */
    removeBuff(typeId) {
        const id=this.skillMap.get(typeId);
        if(id == null)return false;
        const success=this.player.events.OnBuffRemovedRequest?.(id);
        if(!success)return false;
        this.skillMap.delete(typeId);
        return true;
    }
    /**
    ...
    clearAll() {
        for(const [typeId] of this.skillMap.entries()){
    ...
    emitEvent(event,params) {
        for(const [, id] of this.skillMap.entries()){
            this.player.events.OnSkillEmitEvent?.(id, event, params);
        }
    }
}
